...
public class LZSS extends Algorithm {
    ...
    public Object[] comprimir(Fitxer inFile, ByteArrayOutputStream compressedFile) throws DomainLayerException {
        ...
        if (content.length > 0) {
            statistics[0] = content.length;
            statistics[1] = compressedContent.length;
            statistics[2] = (double) (endTime - startTime) * 0.001;
            globalStatistic.addTotalCompressedData(content.length);
            globalStatistic.addTotalCompressionTime((double) statistics[2]);
    ...
    public Object[] descomprimir(byte[] content, Fitxer outputFile) throws DomainLayerException{
        ...
        statistics = new Object[]{decompressedContent.length, content.length, decompressTime};
        return statistics;
    }
